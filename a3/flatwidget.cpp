...
void FlatWidget::draw()
{
    ...
    for (QVector<Triangle3>::iterator pit = cylinderList.begin(); pit != cylinderList.end(); pit++)
    {
        for (int i = 0; i < 3; i++)
        {
            ...
            if (!firstPoint)
            {
               drawLine(get2DPoint(QVector3D(prevpSurface.x, prevpSurface.y, prevpSurface.z)), get2DPoint(QVector3D(pSurface.x, pSurface.y, pSurface.z)));
    ...
    {
        point = get2DPoint(it->position);
        drawCircle(RADIUS, point.x(), point.y(), false);
    }
}
...
QVector3D * FlatWidget::pointAt(QVector3D point)
{
    ...
    for (QVector<LightSource>::iterator lit = dataManager->lights.begin(); lit != dataManager->lights.end(); lit++)
    {
        if (pointOnCircle(get2DPoint(point), get2DPoint( lit->position ), (double)RADIUS))
        {
            return &(lit->position);
        }
    }
...
void FlatWidget::drawCylinder()
{
    cylinderList = dataManager->calculateFrame(shape, cylinder_radius).cylinderList;
}

void FlatWidget::toggleSpecial(bool on)
{
    special = on;
}
