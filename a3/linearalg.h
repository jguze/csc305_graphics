...
//    Matrix33 * translate3(Vector3 v);
    ...
    Matrix44 * rotate4X(double angle);

    Matrix44 * rotate4Y(double angle);

    Matrix44 * rotate4Z(double angle);

    Matrix33 * rotateAxisUVW(Vector3 uvw, double angle);

    Matrix44 * rotateLineUVW(Vector3 abc, Vector3 uvw, double angle);

    Matrix44 * scale4(double scalar);

    Matrix44 * translate4(Vector3 v);

    Matrix44 * translate4(double x, double y, double z);
    ...
    Matrix33 rotateAToB(Vector3 a, Vector3 b);
...
public:
    Polygon3() {}
    Polygon3(QVector<Vector3> p) : points(p){}
...
public:
    Vector3temp(){}
    Vector3temp(double _x, double _y, double _z): x(_x), y(_y), z(_z) {};
    double x;
    double y;
    double z;
};
...
public:
    ...
    Matrix33 tensorProduct(Vector3 v);
    ...
    double y;
    double z;
    Vector3temp splinePoint; //SUPER TEMPORARY
};

/**
 * @brief The Vector4 class A Quaternion
 */
class Vector4
{
public:
    ...
    /**
     * @brief operator [] Overloading the square brackets to access the x,y, and z and w values by integer.
     *      0 accesses x, 1 accesses y, and 2 accesses z, and 3 accesses w. Every other value accesses x.
     * @param i
     * @return
     */
    double& operator [](int i);
     ...
     * @brief operator * Multiplies this vector and all it's values by some scalar
     * @param scalar A double to multiply the x, y, and z values by
     * @return A Vector4 where x, y, z, and w are all multiplied by the scalar
     */
...
{
public:
    Triangle3() {x = y = z = Vector3(0,0,0);}
    Triangle3(Vector3 _x, Vector3 _y, Vector3 _z) : x(_x), y(_y), z(_z) {}
...
public:
    ...
    Matrix33 multiply(double scalar);
    ...
    Matrix33 operator *(double scalar) {return multiply(scalar);}

    Matrix33 operator +(Matrix33 m) {return add(m);}
...
public:
    ...
    Matrix44(Vector4 x, Vector4 y, Vector4 z, Vector4 w);
    ...
    Matrix44 operator *(Matrix44 m) {return multiply(m);}

    Vector4 operator *(Vector4 v) {return multiply(v);}
...
{
public:
    Square3() {btmLeft = btmRight = topLeft = topRight = Vector3(0,0,0);}
    Square3(Vector3 _bl, Vector3 _br, Vector3 _tl, Vector3 _tr) :btmLeft(_bl), btmRight(_br), topLeft(_tl), topRight(_tr) {}

    Square3 multiply(Matrix33 m);

    Square3 operator *(Matrix33 m) {return multiply(m);}
    Vector3& operator [](int i);
...
class FrenetCrossSection
{
public:
    FrenetCrossSection() {}

    FrenetCrossSection(Vector3 _curvePoint, Vector3 velocity, Vector3 acceleration, Vector2 __c)
    {
        curvePoint = _curvePoint;
        _c = __c;
        tangent = velocity.normalize();
        normal = velocity.crossProduct(acceleration).crossProduct(velocity).normalize();
        binormal = tangent.crossProduct(normal);
        calculateSurfacePoint();
    }
    ...
    Vector3 calculatePoint(Vector3 v)
    {
//        return v * Matrix33(normal, binormal, curvePoint);
        return Vector3( curvePoint.x + v.x * normal.x + v.y * binormal.x, curvePoint.y + v.x * normal.y + v.y * binormal.y, curvePoint.z + v.x * normal.z + v.y * binormal.z);
    }
...
private:
    ...
    {
        surfacePoint = calculatePoint(Vector3(_c.x, _c.y, 1));
//         surfacePoint = Vector3( curvePoint.x + _c.x * normal.x + _c.y * binormal.x, curvePoint.y + _c.x * normal.y + _c.y * binormal.y, curvePoint.z + _c.x * normal.z + _c.y * binormal.z);
    }
...
public:
    ...
    FrenetCrossSection * calcFrame(double t, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, double radius)
    {
        Vector3 p = calcSpline(t, p0, p1, p2, p3);
        Vector3 velocity = calcVelocity(t, p0, p1, p2, p3);
        Vector3 acceleration = calcAcceleration(t, p0, p1, p2, p3);
        return new FrenetCrossSection(p, velocity, acceleration, Vector2(radius,radius));
    }

    Vector3 calcVelocity(double t, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3)
    {
        return (
                      ( ( (p0 * -1) + p2 ) * tension) +
                      (( (p0 * 2 * tension) + (p1 * (tension - 3)) + (p2 * (3 - 2 * tension)) + (p3 * -1 * tension) ) * t * 2) +
                      ( (p0 * -1 * tension) + (p1 * (2 - tension)) + (p2 * (tension - 2)) + p3 * tension) * t * t * 3);
    }

    Vector3 calcAcceleration(double t, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3)
    {
        return (
                      (( (p0 * 2 * tension) + (p1 * (tension - 3)) + (p2 * (3 - 2 * tension)) + (p3 * -1 * tension) ) * 2) +
                      ( (p0 * -1 * tension) + (p1 * (2 - tension)) + (p2 * (tension - 2)) + p3 * tension) * t * 6);
    }
};
